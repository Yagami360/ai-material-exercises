# ai-material-exercises

AI 創薬 / AI マテリアル分野のモデルやフレームワークの練習用レポジトリ

## NVIDIA Clara

- [[In-progress] NVIDIA Clara の概要](1/)

- NVIDIA Clara for Biopharma（AI 創薬）
    - [[In-progress] NVIDIA Clara for Biopharma / BioNeMo の概要](2/)
    - [NVIDIA BioNeMo の blueprint デモからAI創薬における入出力データを理解する](3/)
    - [BioNeMo Framework をインストールする](4/)
    - [[In-progress] BioNeMo Framework を使用して AI 創薬モデル（ESM-2）の学習を行う](5/)
    - BioNeMo Framework を使用して AI 創薬モデル（ESM-2）の推論を行う

- NVIDIA Clara for Medical Imaging（AI 医療用画像診断）
